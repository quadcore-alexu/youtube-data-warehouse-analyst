...
if __name__ == '__main__':
    ...
    while True:
        ...
        silver_countries_table = (spark
                              .read
                              .format("delta")
                              .load("hdfs://namenode:9000/tmp/silver_ages")
                              .groupBy("channel_id", "age")
                              .agg(sum("views_count").alias("views_count"), sum("likes_count").alias("likes_count"), sum("minutes_count").alias("minutes_count"))
                              .select("channel_id", "age", "views_count", "likes_count", "minutes_count"))

        # Merge the aggregated data into the silver table
        (gold_table.alias("gold")
         .merge(silver_countries_table.alias("silver"), "gold.channel_id = silver.channel_id and gold.age = silver.age")
