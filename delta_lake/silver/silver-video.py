...
if __name__ == '__main__':
    ...
    while True:
        ...
        bronze_first_views_table = bronze_first_views_table.where((col("timestamp_seconds") >= first_views_start_timestamp) & (
            col("timestamp_seconds") < first_views_end_timestamp)).withColumn("hour_timestamp_seconds", col('timestamp_seconds') - col('timestamp_seconds') % lit(3600))
        ...
        bronze_likes_table = bronze_likes_table.where((col("timestamp_seconds") >= likes_start_timestamp) & (
            col("timestamp_seconds") < likes_end_timestamp)).withColumn("hour_timestamp_seconds", col('timestamp_seconds') - col('timestamp_seconds') % lit(3600))
        ...
        bronze_view_actions_table = bronze_view_actions_table.where((col("timestamp_seconds") >= view_actions_start_timestamp) & (
            col("timestamp_seconds") < view_actions_end_timestamp)).withColumn("hour_timestamp_seconds", col('timestamp_seconds') - col('timestamp_seconds') % lit(3600))
        ...
        (silver_table.alias("silver")
         .merge(aggregated_data.alias("bronze"), "silver.video_id = bronze.video_id and silver.hour_timestamp = bronze.hour_timestamp_seconds")
